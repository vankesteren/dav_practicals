...
1. __Create a linear model object called `lm_ses` using the formula `medv ~ lstat` and the `Boston` dataset__
...
# for each point increase in lstat, the median housing value 
# drops by 0.95
```
...
6. __Use the `seq()` function to generate a sequence of 1000 equally spaced values from 1 to 40. Store this vector in a data frame (`data.frame()` or `tibble()`) with as its column name `lstat`. Name the data frame `pred_dat`__
...
7. __Use the newly created data frame as the `newdata` argument to a `predict()` call for `lm_ses`. Store it in a variable named 6. `y_pred_new`.__
...
25. __Create a second model `model_2` which includes `age` and `tax` as predictors. Calculate the train and validation MSE.__
...
cv_lm <- function(formula, dataset, k) {
  ...
  for (i in 1:k) {
    ...
    mses[i] <- mse(y_true = data_valid %>% select(y_column_name),
