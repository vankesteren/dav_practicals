...
> __Open RStudio on your computer, and go to Tools > Global options to change the appearance settings to your preference__
...
> __Run the following in the R console__
...
> __Get familiar with help files by searching for a few more functions, such as `plot()`, `lm()`, and `print()`__

Everyone uses help files extensively, even the most prolific `R` programmers. 

> __Try to find out what the `prop.table()` function does by searching for it on the internet__
...
> __Open the file `01 - R basics for DAV.Rproj` in RStudio and run the following code in the console. Do you know where the file "sometext.txt" is located on your disk?__
...
> __In this course (and beyond!), always work in a project. For the remainder of this practical, we will work in the project `01 - R basics for DAV`.__
...
> __Open the file `r_basics.Rmd` from the Files pane in RStudio.__
...
> __Read through the following tutorials on rmarkdown.rstudio.com.__
...
> __Play around with the default R Markdown template that appears when you create a new .Rmd file via File > New File > R Markdown.__

RStudio may ask you to install several packages. You should allow it to!

> __Make sure you can output the file you created to a pdf using Knit > Knit to pdf.__
...
For the practicals, we will always start by opening the R project and do the assignments directly in the R Markdown file.
...
> __Install and load the package in `R` by running the following in the console__
...
> __Look at the Default dataset by running the following in the console. What does this dataset contain?__
...
> __Install and load the packages from the tidyverse by running the following in the console__
...
You can see that tidyverse contains many packages, some of which are loaded by default when tidyverse is loaded. We are mostly going to use `ggplot2` and `dplyr`. In the next practical, we are going to learn about how to use `dplyr` for data manipulation.
...
> __Read throught the [Google style guide](https://google.github.io/styleguide/Rguide.xml) for R__
...
If you have followed this practical, you are all set for the remainder of this course! For the next practical (also in this week!), open the file `02 - Data manipulation.Rproj` from its folder.
